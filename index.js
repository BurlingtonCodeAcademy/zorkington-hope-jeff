...
const officerQuarters = new Room(
  "Officer Quarters",
...
class Item {
  constructor(name, description, takeable) {
    (this.name = name), (this.desc = description), (this.takeable = takeable);
  }
}
...
const energyDrink = new Item(
  "Energy Drink",
  "That energy drink sure looks refreshing.",
  true
);
const coveralls = new Item(
  "coveralls",
  "Coveralls are worn by the enlisted staff.",
  false
);
const ECM = new Item(
  "ECM",
  "The ECM is necessary to switch fuels for a quantum jump.",
  true
);
const coffeeCup = new Item(
  "Coffee Cup",
  "Caffeine makes the days fly by.",
  false
);
const journal = new Item(
  "Journal",
  "This is a daily record kept by officers.",
  false
);
...
const blueprint = new Item(
  "Blueprint",
  "At the bottom of the blueprint, it reads: thanks for all the fish.",
  true
);
const key = new Item(
  "Officer Quarters key",
  "This key grants access to the Officer Quarters.",
  true
);
...
let shipRooms = {
  ...
  messHall: {
    canChangeTo: ["generatorRoom", "officerQuarters", "engineBlock", "hangar"],
  },
...
async function moveToNewRoom(newRoom) {
  ...
  if (changeRoom.includes(newRoom)) {
    ...
    if (player.inventory.includes("ECM") && player.location === "hangar") {
      console.log(
        `You rush back to your spacecraft in the hangar and immediately begin installation of your new ECM. With not a moment to spare you fix your drive, take off from the hangar, and begin your Quantum jump back home! \n\n!!!VICTORY!!!\n\nNow you're expected back at work on Monday...so "victory" is a subjective word `
      );
      process.exit();
    }
  } else {
...
async function launch() {
  ...
  if (answer !== "yes") {
    console.log(
      `*static radio* Incoming message from space pirates, 'I have you now!'...\nYou\'re disintegrated into space particles `
    );
    process.exit();
  } else {
    console.log(
  ...
  while (true) {
    ...
    let question = await ask(
      "move, drink, take, drop, check inventory, read, status\nWhat do you want to do? "
    );
    ...
    // This is the 'take' action and it's respective 'await / ask'
      ...
      if (room.inv.includes(takeQ)) {
        ...
        room.inv.splice(roomIndex, 1);
      } else {
        console.log(
          "That specific item is not found in this room. Look elsewhere. "
        );
      }
    }
    ...
    // this is the 'drop' action for dropping player items
      ...
      if (player.inventory.includes(dropQ)) {
        ...
        let presentInv = player.inventory.indexOf(dropQ);
        console.log(presentInv);
        player.inventory.splice(presentInv, 1);
        room.inv.push(dropQ);
      } else {
    ...
    // This is the 'check inventory' action and it's assigned 'await / ask'
      ...
      if (inventoryQ === "yes") {
        console.log(
          "The items you have in your inventory are: " + player.inventory
        );
      } else {
        console.log("\n\nOK ... moving on ");
      }
    ...
    // This is the 'drink' action and it's assigned 'await / ask'
      ...
      let indexNum = player.inventory.indexOf("energyDrink");
      ...
      if (drinkQ === "yes" && player.inventory.includes("energyDrink")) {
        ...
        player["changeStatus"]();
        player.inventory.splice(indexNum, 1);
        console.log("\n\nYou are currently feeling: " + player.status);
      } else if (drinkQ === "no") {
