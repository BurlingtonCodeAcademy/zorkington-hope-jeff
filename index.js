...
async function moveToNewRoom(newRoom) {
  ...
  if (changeRoom.includes(newRoom)) {
    ...
    if (player.inventory.includes('ECM')){
...
async function launch() {
  ...
  if (answer !== 'yes') {
    console.log(`*static radio* Incoming message from space pirates, 'I have you now!'... You\'re disintegrated into space particles `)
    process.exit()
  } else {
  ...
  if (blueprintQ === 'leave'){
    ...
    console.log('\nOh no, the poison left on the blueprint spreads throughout your body and you have now died, INCONCEIVABLE! ')
    process.exit()
  }
  ...
  while (true) {
    ...
    if (question === 'move') {
      ...
      if (drinkQ === 'yes') {  // why does player.inv not work here?
